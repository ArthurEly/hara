...
def parse_submodule_utilization_with_onnx_modified():
    ...
    """
    ### ALTERAÇÃO 1: Converta o caminho único para uma lista de caminhos. ###
    # Adicione todos os diretórios base que você deseja pesquisar nesta lista.
    base_directories = [
        "/home/arthurely/Desktop/finn/hara/builds",
        "/home/arthurely/Desktop/finn/notebooks/sat6_cnn/builds_jics",
    ...
    all_submodules_data_for_complete_csv = []
    found_files_count = 0
    all_seen_onnx_attr_keys = set()

    submodule_type_pattern = re.compile(r'^(.*?)_(rtl|hls)_(\d+)$')

    ### ALTERAÇÃO 2: Adicione um loop para iterar sobre cada diretório base. ###
    for base_directory in base_directories:
        print(f"\n--- A iniciar pesquisa no diretório base: '{base_directory}' ---")

        if not os.path.isdir(base_directory):
            print(f"Erro: Diretório '{base_directory}' não encontrado. A saltar para o próximo.")
            continue
        ...
        for hw_config_dir_path in potential_hw_config_dirs:
            hardware_config_name = f"{os.path.basename(base_directory)}_{os.path.basename(hw_config_dir_path)}" # Nome mais único
                        ...
                        if not found_finn_design_i:
                            ...
                            if current_indent == finn_design_i_indent_level + 2:
                                ...
                                try:
                                    ...
                                    all_submodules_data_for_complete_csv.append(row_dict)
    ...
    # --- O restante do código para salvar os CSVs permanece fora do loop ---
    # Ele será executado uma vez após todos os diretórios base terem sido processados.

    if found_files_count == 0:
        print(f"Nenhum ficheiro '{report_filename_to_search}' encontrado em nenhum dos diretórios fornecidos.")
        return
    if not all_submodules_data_for_complete_csv:
    ...
    if all_submodules_data_by_base_name:
        ...
        elif all_submodules_data_by_base_name:
