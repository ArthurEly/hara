...
# (Esta função auxiliar é a mesma que estava no script anterior)
def _run_estimate_build(base_build_dir, onnx_model_path, hw_name, topology_id, quant, folding_path=None, target_fps=None):
    # ... (código da função _run_estimate_build, sem alterações)
    print(f"--- Executando Estimativa para: {hw_name} ---")
    build_output_dir = os.path.join(base_build_dir, hw_name)
    steps = BUILD_CONFIG['first_run_estimate']['steps']
    args = [
        "python3", "./hara/run_build.py", "--model_path", str(onnx_model_path),
        "--build_dir", str(base_build_dir), "--topology", str(topology_id),
        "--quant", str(quant), "--steps", json.dumps(steps), "--hw_name", hw_name,
...
def generate_map(model_info, base_build_dir):
    # ... (código da função generate_map, sem alterações)
    ...
    est_dir_1 = _run_estimate_build(base_build_dir, master_onnx_path, "run0_get_initial_fold", topology_id, quant, target_fps=1)
    ...
    baseline_dir = _run_estimate_build(base_build_dir, master_onnx_path, baseline_hw_name, topology_id, quant, folding_path=folding_path)
    ...
    while True:
        ...
        current_build_dir = _run_estimate_build(base_build_dir, master_onnx_path, current_hw_name, topology_id, quant, folding_path=folding_path)
    ...
    plt.title(f"Curva de Performance Teórica para {topology_id}")
...
if __name__ == "__main__":
    ...
    # Este script agora também aceita --build_dir
    ...
    generate_map(model_info, args.build_dir)